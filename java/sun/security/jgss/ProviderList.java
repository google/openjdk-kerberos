...
public final class ProviderList {
    ...
    public ProviderList(GSSCaller caller, boolean useNative) {
        this.caller = caller;
        Provider[] provList;
        GSSUtil.debug("In ProviderList, use native? " + useNative);
        if (useNative) {
        ...
        GSSUtil.debug("Adding SunProvider");
        try {
            addProviderAtEnd(SunProvider.INSTANCE, null);
        } catch (GSSException ge) {
            // Move on to the next provider
            GSSUtil.debug("Error in adding provider " +
                    SunProvider.INSTANCE.getName() + ": " + ge);
        }
    }
